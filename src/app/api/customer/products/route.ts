import { createApiHandler } from "@/lib/api/api-handler";
import { apiSuccess } from "@/lib/api/api-response";
import { catalogService } from "@/features/customers/services/catalog.service";
import {
  customerProductListSchema,
  type CustomerProductListInput,
} from "@/features/customers/validations/catalog.schema";

export const POST = createApiHandler(
  {
    POST: async (_request, context) => {
      const body = (context.body || {}) as CustomerProductListInput;
      const result = await catalogService.getProducts(body);
      return apiSuccess(result.data, "Products fetched successfully", 200, result.meta);
    },
  },
  {
    bodySchema: customerProductListSchema,
  }
);
