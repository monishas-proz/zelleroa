...
export const POST = createApiHandler(
  {
    POST: async (_request, context) => {
      ...
      const body = context.body as MarkFailedInput;
      const result = await deliveryService.markFailed(
        sessionUserId,
        parsedParam.data.uuid,
        body
      );

      return apiSuccess(result, "Delivery marked as failed", 200);
    },
  },
  {
    requireAuth: true,
    requiredRole: ["STAFF"],
    bodySchema: markFailedSchema,
  }
);
