# Customer Module Development Guide & Agent Skill (`customer_agent.md`)

This guide contains the complete development rules, architectural standards, API patterns, UX guidelines, and full API endpoint specifications established for the **Customer Module** of Rithu Snacks.

---

## 1. Strict Isolation & Safety Boundaries

1. **Customer Module Scope Only:**
   - All customer-facing code belongs exclusively in `src/features/customers/`, customer account pages (e.g., `src/app/(customer)/`, `/profile`), and customer API routes (`/api/customer/*`).
...
1. **TanStack React Query for All APIs:**
   ...
   - Always invalidate relevant query keys upon successful mutations (e.g., `queryClient.invalidateQueries({ queryKey: customerAddressKeys.all })`).
2. **Send Only Dirty / Modified Fields on Edit (Delta Payloads):**
...
3. **Double-Hit Prevention & Active Loading States:**
...
4. **Exact Zod Schema Validation & Live Error Display:**
